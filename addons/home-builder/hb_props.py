...
def update_library_tab(self,context):
    ...
    if library:
        ...
        # Refresh the asset library so Blender re-scans the path
        if bpy.ops.asset.library_refresh.poll():
            bpy.ops.asset.library_refresh()

        # Also refresh our custom asset list for the UI
...
class HB_Asset_Item(PropertyGroup):
    """Represents a single asset entry in our custom asset list.
    Replaces the removed bpy.types.AssetHandle."""
    asset_name: StringProperty(name="Asset Name")
    asset_library_path: StringProperty(name="Library Path",
        description="Path to the library .blend file containing this asset")
    preview_id: IntProperty(name="Preview ID", default=0,
        description="ID of the preview image in bpy.utils.previews")
...
class Home_Builder_Window_Manager_Props(PropertyGroup):
    ...
    def get_active_asset(self,context):
        ...
        if workspace.home_builder_library_index < len(self.home_builder_library_assets):
            return self.home_builder_library_assets[workspace.home_builder_library_index]
        return None

    def refresh_library_assets(self, context):
        """Scan the active library .blend file and populate home_builder_library_assets."""
        # Clear existing assets
        self.home_builder_library_assets.clear()

        library = self.get_active_library(context)
        if not library:
            return
        ...
        # Load asset names from the blend file
        try:
            with bpy.data.libraries.load(library_path) as (data_from, data_to):
                # Assets are marked on objects in the library file
                asset_names = list(data_from.objects)
                # Also check materials (for material libraries)
                material_names = list(data_from.materials)
        except Exception as e:
            print(f"Home Builder: Error loading library {library_path}: {e}")
            return

        # For material libraries, list materials; for everything else, list objects
        if library.library_type == 'MATERIALS':
            for mat_name in sorted(material_names):
                item = self.home_builder_library_assets.add()
                item.name = mat_name
                item.asset_name = mat_name
                item.asset_library_path = library_path
        else:
            for obj_name in sorted(asset_names):
                item = self.home_builder_library_assets.add()
                item.name = obj_name
                item.asset_name = obj_name
                item.asset_library_path = library_path

        # Clamp the index
        workspace = context.workspace.home_builder
        if workspace.home_builder_library_index >= len(self.home_builder_library_assets):
            workspace.home_builder_library_index = max(0, len(self.home_builder_library_assets) - 1)
