...
def get_active_asset_info(context):
    """Get the active asset name and library from our custom asset list.
    This replaces context.asset (AssetHandle) which was removed in Blender 5.0."""
    wm_props = context.window_manager.home_builder
    workspace = context.workspace.home_builder
    index = workspace.home_builder_library_index
    if index < len(wm_props.home_builder_library_assets):
        asset_item = wm_props.home_builder_library_assets[index]
        library = wm_props.get_active_library(context)
        return asset_item.asset_name, library
    return None, None
...
class home_builder_OT_drop_material(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.mat is None:
            self.report({'WARNING'}, "No material selected in library")
            return {'CANCELLED'}
    ...
    def get_material(self,context):
        asset_name, library = get_active_asset_info(context)
        if asset_name and library:
            return pc_utils.get_material(library.library_path, asset_name)
        return None
...
class home_builder_OT_drop_decoration(bpy.types.Operator):
    ...
    def get_object(self,context):
        asset_name, library = get_active_asset_info(context)
        if not asset_name or not library:
            return
        path = os.path.join(os.path.dirname(library.library_path),'assets', asset_name + ".blend")
...
class home_builder_OT_lookup_drop_id(bpy.types.Operator):
    ...
    def execute(self, context):
        asset_name, library = get_active_asset_info(context)
        if asset_name:
            # Try to find drop_id from the library
            wm_props = context.window_manager.home_builder
            active_library = wm_props.get_active_library(context)
            if active_library and active_library.drop_id:
                eval("bpy.ops." + active_library.drop_id + "()")
        return {'FINISHED'}
