...
class home_builder_OT_create_new_project(bpy.types.Operator):
    bl_idname = "home_builder.create_new_project"
    ...
    room_name: bpy.props.StringProperty(name="Room Name",
                                   description="Enter Room Name",
                                   default="Room Name")

    use_current_file: bpy.props.BoolProperty(name="Use Current File",
                                   description="This will save the current file you have open as the room",
                                   default=True)
    ...
    def get_project_path(self):       
        docs_path = hb_paths.get_project_folder_path()
        project_folder_name = self.project_name
        return os.path.join(docs_path,project_folder_name)

    def get_filepath(self,project_path):
        return os.path.join(project_path,self.room_name + ".blend")

    def execute(self, context):
        project_path = self.get_project_path()
        if not os.path.exists(project_path):
            os.makedirs(project_path)
        filepath = self.get_filepath(project_path)
        if not self.use_current_file:
    ...
    def draw(self, context):
        ...
        row.prop(self,'room_name',text="")
        row = layout.row()
        row.label(text="Use Current File")
        row.prop(self,'use_current_file',text="")        
        project_path = self.get_project_path()
        filepath = self.get_filepath(project_path)
        if os.path.exists(project_path):
            layout.label(text="Project Path Already Exists",icon='ERROR')
        if os.path.exists(filepath):
            layout.label(text="Room Path Already Exists",icon='ERROR')
        if is_name_invalid(self.room_name) or is_name_invalid(self.project_name):
            layout.label(text="Field cannot contain",icon='ERROR')
            layout.label(text="< > : '' / | ? *",icon='BLANK1')


class home_builder_OT_open_project(bpy.types.Operator, ImportHelper):
    """Open Project"""
    bl_idname = "home_builder.open_project"
...
class home_builder_OT_add_room(bpy.types.Operator):
    bl_idname = "home_builder.add_room"
    bl_label = "Add Room"
    bl_options = {'UNDO'}
    ...
    def draw(self, context):
        ...
        project_path = self.get_project_path()
        filepath = self.get_filepath(project_path)
        if is_name_invalid(self.room_name):
            layout.label(text="Room Name cannot contain",icon='ERROR')
            layout.label(text="< > : '' / | ? *",icon='BLANK1')
        if os.path.exists(filepath):
            layout.label(text="Room Path Already Exists",icon='ERROR')
...
class home_builder_OT_load_library(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if library:
            ...
            # Refresh our custom asset list for the sidebar UI
            wm_props = context.window_manager.home_builder
...
classes = (
    home_builder_OT_create_new_project,
    home_builder_OT_open_project,
    home_builder_OT_add_room,
