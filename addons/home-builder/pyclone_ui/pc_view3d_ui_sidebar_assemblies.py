...
def draw_cage_properties(context, layout, cage):
    ...
    box = col.box()
    row = box.row()
    row.label(text="Cage Name: " + cage.obj.name)
    ...
    if scene_props.assembly_tabs == 'MAIN':
        row = box.row()
        row.prop(cage.obj,'name')
        row.prop(cage.obj,'hide_viewport',text="")

        node = cage.mod.node_group
        col = box.column(align=True)
        col.label(text="Dimensions:")
        row = col.row(align=True)

        cage.draw_input(row,"Dim X","X")
        cage.draw_input(row,"Mirror X","",icon='MOD_MIRROR')

        row = col.row(align=True)
        cage.draw_input(row,"Dim Y","Y")
        cage.draw_input(row,"Mirror Y","",icon='MOD_MIRROR')        

        row = col.row(align=True)
        cage.draw_input(row,"Dim Z","Z")
        cage.draw_input(row,"Mirror Z","",icon='MOD_MIRROR')         

        col = box.column()
        s_col = col.split()
        s_col.prop(cage.obj,'location')
        s_col.prop(cage.obj,'rotation_euler',text="Rotation")

    if scene_props.assembly_tabs == 'PROMPTS':
        cage.obj.pyclone.draw_prompts(box)
    ...
    if scene_props.assembly_tabs == 'OBJECTS':
        ...
        for child in cage.obj.children:
...
class VIEW3D_PT_pc_cage_properties(bpy.types.Panel):
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "Cage"
    bl_category = "Item"    
    ...
    def poll(cls, context):
        if context.object:
            if context.object.pyclone.get_geo_node_cage():
                return True
            
            if context.object.parent:
                if context.object.parent.pyclone.get_geo_node_cage():
                    return True
    ...
    def draw(self, context):
        layout = self.layout
        cage = context.object.pyclone.get_geo_node_cage()

        if cage == None:
            cage = pc_types.GeoNodeCage(context.object.parent)

        draw_cage_properties(context,layout,cage)   
