...
def draw_driver(layout,obj,driver):
    ...
    cage = None
    if obj.parent:
        cage = obj.parent.pyclone.get_geo_node_cage()

    override_cage = None
    if props.driver_override_object:
        override_cage = props.driver_override_object.pyclone.get_geo_node_cage()
        override_obj_bp = pc_utils.get_assembly_bp(props.driver_override_object)
        if override_cage:
            var_name = override_cage.obj.name
        elif override_obj_bp:
    ...
    if props.driver_override_object:
        if override_cage:
            ...
            props.var_object_name = override_cage.obj.name
        ...
        props = row.operator('pc_driver.get_vars_from_object',text="Get Variables...",icon='DRIVER')
        props.object_name = obj.name
        props.var_object_name = cage.obj.name
        props.data_path = driver.data_path
        props.array_index = driver.array_index
    elif obj_bp:
        assembly = pc_types.Assembly(obj_bp)
