...
class HOMEBUILDER_UL_library_assets(bpy.types.UIList):
    """UIList to display assets from the Home Builder library.
    Replaces the removed UILayout.template_asset_view()."""

    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            row = layout.row(align=True)
            row.label(text=item.asset_name, icon='OBJECT_DATA')
        elif self.layout_type == 'GRID':
            layout.alignment = 'CENTER'
            layout.label(text=item.asset_name, icon='OBJECT_DATA')
...
class HOME_BUILDER_PT_library(bpy.types.Panel):
    ...
    def draw_library(self,layout,context,library):
        if library:
            ...
            if len(wm_props.home_builder_library_assets) == 0:
                wm_props.refresh_library_assets(context)

            # Draw the asset list
            col = layout.column()
            col.template_list(
                "HOMEBUILDER_UL_library_assets", "",
                wm_props, "home_builder_library_assets",
                workspace.home_builder, "home_builder_library_index",
                rows=8,
                type='DEFAULT',
            )

            # Draw activate / drop buttons
            if len(wm_props.home_builder_library_assets) > 0:
                active_index = workspace.home_builder.home_builder_library_index
                if active_index < len(wm_props.home_builder_library_assets):
                    active_asset = wm_props.home_builder_library_assets[active_index]
                    row = col.row(align=True)

                    activate_id = library.activate_id if library.activate_id != "" else ""
                    drop_id = library.drop_id if library.drop_id != "" else ""

                    if activate_id:
                        op = row.operator(activate_id, text="Activate " + active_asset.asset_name, icon='PLAY')
                    if drop_id:
                        op = row.operator(drop_id, text="Place " + active_asset.asset_name, icon='IMPORT')
        else:
            layout.separator()
            layout.operator('home_builder.load_library')

    def draw_project_menus(self,layout):
        project_name = "None"
        room_name = "None"
        if bpy.data.filepath != "":
            project_path = os.path.dirname(bpy.data.filepath)
            room_name = os.path.basename(bpy.data.filepath)
        ...
        if project_name == 'None':
            row.menu('HOME_BUILDER_MT_projects',text="Current Project: " + project_name)
            row.menu('HOME_BUILDER_MT_open_room',text="Current Room: " + room_name)
        else:
            room, ext = os.path.splitext(room_name)
            row.menu('HOME_BUILDER_MT_projects',text=project_name)
            row.menu('HOME_BUILDER_MT_open_room',text=room)

    def draw(self, context):
        layout = self.layout
        scene = context.scene
        hb_scene = scene.home_builder
        b_version = bpy.app.version[0]
        ...
        self.draw_project_menus(layout)

        main_box = layout.box()
        if b_version < 5:
            main_box.label(text="Home Builder 5.0 requires Blender 5.",icon="ERROR")
            main_box.label(text="Download Blender 5 to Continue",icon="BLANK1")
        row = main_box.row()
...
class HOME_BUILDER_MT_projects(bpy.types.Menu):
    bl_label = "Projects"

    def draw(self, context):
        layout = self.layout
        layout.operator('home_builder.create_new_project',icon='ADD')
        layout.separator()
        layout.operator('home_builder.open_project',icon='FILEBROWSER')


class HOME_BUILDER_MT_open_room(bpy.types.Menu):
    bl_label = "Open Room"
    ...
    def draw(self, _context):
        ...
        for f in file_list:
            ...
            if ext.upper() == '.BLEND':
                icon = 'CHECKBOX_DEHLT'
                if current_filename == f:
                    icon = 'CHECKBOX_HLT'
                props = layout.operator('wm.open_mainfile',text=filename,icon=icon)
                props.filepath = os.path.join(project_path,f)
                props.display_file_selector = False
        layout.separator()
        layout.operator('home_builder.add_room',text="Add New Room",icon='ADD')
...
classes = (
    ...
    HOME_BUILDER_MT_projects,
    HOME_BUILDER_MT_open_room,
