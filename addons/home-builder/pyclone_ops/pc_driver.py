...
class DRIVER_OT_get_vars_from_object(Operator):
    ...
    def get_prompts(self):
        ...
        if self.cage:
            for prompt in self.cage.obj.pyclone.prompts:
    ...
    def get_var_from_assembly(self):
        ...
        for DR in drivers:
            if self.data_path in DR.data_path and DR.array_index == self.array_index:
                ...
                if self.x_loc:
                    var = DR.driver.variables.new()
                    var.name = 'loc_x'
                    var.targets[0].id = self.cage.obj
                    var.targets[0].data_path = 'location.x'
                if self.y_loc:
                    var = DR.driver.variables.new()
                    var.name = 'loc_y'
                ...
                if self.z_rot:
                    ...
                    var.targets[0].data_path = 'rotation_euler.z'
                if self.x_dim:
                    new_var = self.cage.get_dim_x_var("dim_x")
                    var = DR.driver.variables.new()
                    var.name = new_var.name
                    var.targets[0].id = new_var.obj
                    var.targets[0].data_path = new_var.data_path
                if self.y_dim:
                    new_var = self.cage.get_dim_y_var("dim_y")
                    var = DR.driver.variables.new()
                    var.name = new_var.name
                    var.targets[0].id = new_var.obj
                    var.targets[0].data_path = new_var.data_path
                if self.z_dim:
                    new_var = self.cage.get_dim_z_var("dim_z")
                    var = DR.driver.variables.new()
                    var.name = new_var.name
                    var.targets[0].id = new_var.obj
                    var.targets[0].data_path = new_var.data_path     
    ...
    def execute(self, context):
        if self.cage:
            self.get_var_from_cage()
        else:
            self.get_var_from_assembly()
    ...
    def invoke(self,context,event):
        ...
        cage = var_obj.pyclone.get_geo_node_cage()
        if cage:
            self.cage = cage
        else:
