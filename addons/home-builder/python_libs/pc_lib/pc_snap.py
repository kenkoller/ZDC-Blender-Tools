...
class Drop_Operator(bpy.types.Operator):
    ...
    def get_selected_cage(self):
        if self.hit_object:
            cage = self.hit_object.pyclone.get_geo_node_cage()
            if cage == None and self.hit_object.parent:
                cage = self.hit_object.parent.pyclone.get_geo_node_cage()
            return cage

    def has_height_collision(self,active_assembly,assembly):
        assembly1_z_1 = active_assembly.obj_bp.matrix_world[2][3]
        assembly1_z_2 = active_assembly.obj_z.matrix_world[2][3]
        assembly2_z_1 = assembly.obj_bp.matrix_world[2][3]
        assembly2_z_2 = assembly.obj_z.matrix_world[2][3]
        
        if assembly1_z_1 >= assembly2_z_1 and assembly1_z_1 <= assembly2_z_2:
            return True
            
        if assembly1_z_2 >= assembly2_z_1 and assembly1_z_2 <= assembly2_z_2:
            return True

        if assembly2_z_1 >= assembly1_z_1 and assembly2_z_1 <= assembly1_z_2:
            return True
            
        if assembly2_z_2 >= assembly1_z_1 and assembly2_z_2 <= assembly1_z_2:
            return True

    def has_cage_height_collision(self,obj_1,obj_2):
        if 'GeoNodeName' in obj_1:
            geo_node_cage = pc_types.GeoNodeCage(obj_1)
            assembly1_z_1 = geo_node_cage.obj.matrix_world[2][3]
            assembly1_z_2 = assembly1_z_1 + geo_node_cage.get_input("Dim Z")
        elif 'IS_ASSEMBLY_BP' in obj_1:
            assembly_1 = pc_types.Assembly(obj_1)      
            assembly1_z_1 = assembly_1.obj_bp.matrix_world[2][3]
            assembly1_z_2 = assembly_1.obj_z.matrix_world[2][3]

        if 'GeoNodeName' in obj_2:
            geo_node_cage = pc_types.GeoNodeCage(obj_2)
            assembly2_z_1 = geo_node_cage.obj.matrix_world[2][3]
            assembly2_z_2 = assembly2_z_1 + geo_node_cage.get_input("Dim Z")
        elif 'IS_ASSEMBLY_BP' in obj_2:
            assembly_2 = pc_types.Assembly(obj_2)      
            assembly2_z_1 = assembly_2.obj_bp.matrix_world[2][3]
            assembly2_z_2 = assembly_2.obj_z.matrix_world[2][3]
    ...
    def get_left_collision_location_and_assembly(self,wall,snap_line,collision_assembly=None,z_override=-1):
        ...
        if z_override != -1:
            search_point_left = (snap_line.location.x,0,z_override)
        else:
            search_point_left = (snap_line.location.x,0,self.hit_location[2])
    ...
    def get_right_collision_location_and_assembly(self,wall,snap_line,collision_assembly=None,z_override=-1):
        self.snap_line.location = self.hit_location
        self.snap_line.location.z = wall.obj_bp.location.z
        self.snap_line.parent = wall.obj_bp
        self.snap_line.matrix_world[0][3] = self.hit_location[0]
        ...
        if z_override != -1:
            search_point_right = (snap_line.location.x,0,z_override)
        else:
            search_point_right = (snap_line.location.x,0,self.hit_location[2])
    ...
    def get_left_cage_collision_location_and_assembly(self,wall,snap_line,collision_cage=None,z_override=-1):
        self.snap_line.location = self.hit_location
        self.snap_line.location.z = wall.obj_bp.location.z
        self.snap_line.parent = wall.obj_bp
        self.snap_line.matrix_world[0][3] = self.hit_location[0]
        ...
        if z_override != -1:
            ...
            search_point_left = (snap_line.location.x,0,self.hit_location[2])
        left_cage = pc_placement_utils.get_left_wall_collision_cage_from_selected_point(wall,search_point_left,ignore_cage = collision_cage)
        snap_points = pc_placement_utils.get_snap_points(wall)
        left_sp = self.get_left_snap_point(snap_points)

        #GET LEFT SNAP LOCATION
        cal_left_x_snap = 0
        if left_cage and left_sp:
            left_cage_x_snap = left_cage.obj.location.x + left_cage.get_input("Dim X")
            left_snap_location = left_sp.location.x
            if left_cage_x_snap > left_snap_location:
                cal_left_x_snap = left_cage_x_snap
            else:
                cal_left_x_snap = left_snap_location
        elif left_cage:
            cal_left_x_snap = left_cage.obj.location.x + left_cage.get_input("Dim X")
        elif left_sp:
            cal_left_x_snap = left_sp.location.x
        else:
            #CHECK NEXT WALL
            ...
            if left_wall_bp:
                left_wall = pc_types.Assembly(left_wall_bp)
                if self.wall_is_inside_corner(wall,'LEFT'):
                    left_cages = pc_placement_utils.get_wall_cages(left_wall)
                    left_cages.reverse()
                    #LOOK FOR ASSEMBLY ON NEXT WALL
                    for cage_bp in left_cages:
                        cage = pc_types.GeoNodeCage(cage_bp)
                        cage_height =  cage.get_input("Dim Z")
                        cage_z_loc = cage.obj.location.z                        
                        #IF HAS HEIGHT COLLISION
                        if self.hit_location[2] > cage_z_loc and self.hit_location[2] < (cage_z_loc + cage_height):
                            if collision_cage:
                                cage_depth = collision_cage.get_input("Dim Y")
                            else:
                                cage_depth = 0
                            #IF ASSEMBLY IS CLOSE ENOUGH TO END OF WALL
        ...
        return cal_left_x_snap, left_cage
    
    def get_right_cage_collision_location_and_assembly(self,wall,snap_line,collision_cage=None,z_override=-1):
        self.snap_line.location = self.hit_location
        self.snap_line.location.z = wall.obj_bp.location.z
        self.snap_line.parent = wall.obj_bp
        self.snap_line.matrix_world[0][3] = self.hit_location[0]
        ...
        if z_override != -1:
            ...
            search_point_right = (snap_line.location.x,0,self.hit_location[2])
        right_cage = pc_placement_utils.get_right_wall_collision_cage_from_selected_point(wall,search_point_right,ignore_cage = collision_cage)
        snap_points = pc_placement_utils.get_snap_points(wall)
        right_sp = self.get_right_snap_point(snap_points)

        cal_right_x_snap = 0
        if right_cage and right_sp:
            if right_cage.obj.location.x < right_sp.location.x:
                cal_right_x_snap = right_cage.obj.location.x
            else:
                cal_right_x_snap = right_sp.location.x
        elif right_cage:
            if round(right_cage.obj.rotation_euler.z,3) == round(math.radians(-90),3):
                cal_right_x_snap = right_cage.obj.location.x - right_cage.get_input("Dim Y")
            else:
                cal_right_x_snap = right_cage.obj.location.x
        elif right_sp:
            cal_right_x_snap = right_sp.location.x
            ...
            if right_wall_bp:      
                right_wall = pc_types.Assembly(right_wall_bp)
                if self.wall_is_inside_corner(wall,'RIGHT'):
                    right_cages = pc_placement_utils.get_wall_cages(right_wall)
                    #LOOK FOR ASSEMBLY ON RIGHT WALL
                    for cage_bp in right_cages:
                        cage = pc_types.GeoNodeCage(cage_bp)
                        cage_height =  cage.get_input("Dim Z")
                        cage_z_loc = cage.obj.location.z
                        #IF HAS HEIGHT COLLISION
