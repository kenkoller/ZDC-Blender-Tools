...
class pc_assembly_OT_create_new_cage(Operator):
    bl_idname = "pc_assembly.create_new_cage"
    bl_label = "Create New Cage"
    bl_description = "This will create a new cage"
    bl_options = {'UNDO'}

    cage_name: StringProperty(name="Assembly Name",default="New Cage")
    ...
    def execute(self, context):
        cage = pc_types.GeoNodeCage()
        cage.create(self.cage_name)
        cage.dim_x(value=pc_unit.inch(24))
        cage.dim_y(value=pc_unit.inch(24))
        cage.dim_z(value=pc_unit.inch(24))
        cage.set_input("Mirror X",False)
        cage.set_input("Mirror Y",False)
        cage.set_input("Mirror Z",False)
        cage.obj.select_set(True)
        # assembly.obj["PROMPT_ID"] = "pc_assembly.show_properties"
        context.view_layer.objects.active = cage.obj
        return {'FINISHED'}
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(text="Enter the name of the cage to add.")
        layout.prop(self,'cage_name')
...
class pc_assembly_OT_add_object(Operator):
    ...
    def add_object_to_assembly(self,context,assembly):
    ...
    def add_object_to_cage(self,context,cage):
        if self.object_type == 'EMPTY':
            cage.add_empty(self.object_name)

        if self.object_type == 'MESH':
            obj_mesh = pc_utils.create_cube_mesh(self.object_name,(cage.get_input("Dim X"),
                                                                   cage.get_input("Dim Y"),
                                                                   cage.get_input("Dim Z")))

            if 'PROMPT_ID' in cage.obj:
                obj_mesh['PROMPT_ID'] = cage.obj['PROMPT_ID']
            
            cage.add_object(obj_mesh)
        ...
        if self.object_type == 'CURVE':
            ...
            spline.bezier_points.add(count=1)                 
            obj_curve.data.splines[0].bezier_points[0].co = (0,0,0)
            obj_curve.data.splines[0].bezier_points[1].co = (cage.get_input("Dim X"),0,0)
            cage.add_object(obj_curve)
        if self.object_type == 'LIGHT':
    ...
    def execute(self, context):
        if context.object and 'GeoNodeName' in context.object:
            cage = pc_types.GeoNodeCage(context.object)
            self.add_object_to_cage(context,cage) 
        elif context.object and context.object.parent and 'GeoNodeName' in context.object.parent:
            cage = pc_types.GeoNodeCage(context.object.parent) 
            self.add_object_to_cage(context,cage)     
        else:
            obj_bp = pc_utils.get_assembly_bp(context.object)
            assembly = pc_types.Assembly(obj_bp)
            self.add_object_to_assembly(context,assembly)
...
classes = (
    ...
    pc_assembly_OT_create_new_cage,
