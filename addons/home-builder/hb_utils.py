...
class _AssetProxy:
    """Lightweight proxy that mimics the old AssetHandle.name interface.
    Allows product drop operators to keep using asset_file_handle.name
    after the migration away from context.asset (removed in Blender 5.0)."""
    def __init__(self, name):
        self.name = name


def get_active_asset_handle(context):
    """Return an AssetProxy for the currently selected asset in the sidebar list.
    This is the Blender 5.0 replacement for context.asset (AssetHandle)."""
    ...
    if index < len(wm_props.home_builder_library_assets):
        ...
        return _AssetProxy(asset_item.asset_name)
    return None
