...
def get_wall_cages(wall):
    """ This returns a sorted list of all of the cages
        parented to the wall
    """
    list_objs = []
    for child in wall.obj_bp.children:
        if "GeoNodeName" in child and child["GeoNodeName"] == 'GeoNodeCage':
...
def get_left_wall_collision_cage_from_selected_point(wall,selected_point,ignore_cage=None):
    list_objs = get_wall_cages(wall)
    cages = []
    for bp in list_objs:
        cages.append(pc_types.GeoNodeCage(bp))
    
    #Collect all Cages to the left
    list_left_cages = []
    for index, cage in enumerate(cages):
        if ignore_cage:
            if cage.obj.name == ignore_cage.obj.name:
                continue
        if (cage.obj.location.x + cage.get_input("Dim X")) < selected_point[0]:
            list_left_cages.append(cage)
    
    #Search Backwards for First Height Collision Cages
    list_left_cages.reverse()
    for left_cage in list_left_cages:
        if selected_point[2] > left_cage.obj.location.z and selected_point[2] < (left_cage.obj.location.z + left_cage.get_input("Dim Z")):
            return left_cage
    return None

def get_right_wall_collision_cage_from_selected_point(wall,selected_point,ignore_cage=None):
    list_objs = get_wall_cages(wall)
    cages = []
    for bp in list_objs:
        cages.append(pc_types.GeoNodeCage(bp))
    ...
    for index, cage in enumerate(cages):
        if ignore_cage:
            if cage.obj.name == ignore_cage.obj.name:
                continue        
        if (cage.obj.location.x + cage.get_input("Dim X")) > selected_point[0]:
            list_right_cages.append(cage)
            
    #Search Forward for First Height Collision Cage
    for right_cage in list_right_cages:
        if selected_point[2] > right_cage.obj.location.z and selected_point[2] < (right_cage.obj.location.z + right_cage.get_input("Dim Z")):
            return right_cage
